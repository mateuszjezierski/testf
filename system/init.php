<?php
	...
	include(BASE_PATH.'/system/libs/coinbase.php');
	include(BASE_PATH.'/system/libs/Faucetpay/faucetpay.php');
	...
	if(isset($_SESSION['PT_User'])){
		...
		$data = $db->QueryFetchArray("SELECT a.*, b.membership AS mem_name, b.multiplier, b.ref_com, b.direct_crypto_manual, b.direct_crypto_auto, b.hide_ads, b.hash_rate, b.bonus_roll, b.withdraw_wait_time, c.hash FROM users a LEFT JOIN memberships b ON b.id = a.membership_id LEFT JOIN users_sessions c ON c.uid = a.id WHERE a.id = '".$ses_id."' AND a.disabled = '0' LIMIT 1");
	...
	/* Get currenct coins value */
	$coin_value = $db->QueryFetchArray("SELECT `value` FROM `coins_value` ORDER BY `time` DESC LIMIT 1");
	$coin_value = unserialize($coin_value['value']);
