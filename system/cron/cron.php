...
$db->Query("DELETE FROM `faucet_sessions` WHERE `time`<'".(time()-1800)."'");

// Get Coins Value
$getCurrencies = $db->QueryFetchArrayAll("SELECT `coingecko_id` FROM `coins` WHERE `status`='1'");
	
$coins = implode(',', array_column($getCurrencies, 'coingecko_id'));
$coins = get_data('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids='.$coins.'&sparkline=false');
...
if(!empty($coins))
{
	...
	{
		$values[$coin['symbol']] = number_format($coin['current_price'], 8, '.', '');
		if(!file_exists(BASE_PATH.'/files/coins/'.$coin['symbol'].'-logo.png'))
		{
			copy($coin['image'], BASE_PATH.'/files/coins/'.$coin['symbol'].'-logo.png');
		}
	}
	$values = serialize($values);
	$db->Query("INSERT INTO `coins_value` (`value`,`time`) VALUES ('".$values."','".time()."')");
}
...
if($cron_5min['time'] < ($timestamp-300)){
	...
	if($write){
		...
		$db->Query("DELETE FROM `coins_value` WHERE `time`<'".(time()-86400)."'");
