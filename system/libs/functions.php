<?php
	...
	function getAchievement($achievement, $requirements, $reward, $reward_type, $membership = 'Membership Days')
	{
		...
		$reward_type = ($reward_type == 0 ? 'Coins' : ($reward_type == 1 ? $membership : '$'));
		$achievements = array(
	...
	function getCurrency($type = 'stock', $coin = 'btc'){
		global $db;

		$getCurrency = $db->QueryFetchArray("SELECT * FROM `coins` WHERE `coin`='".$coin."' LIMIT 1");

		return $getCurrency[$type];
	}

	// Get payment method info
	function paymentMethod($method, $type = 0, $currency = 'btc', $address= '') {
		// 0 = Name, 1 = URL, 2 = Transaction proof
		
		$array = array(
				1 => array('FaucetPay', 'https://faucetpay.io/?r=2233', false),
				2 => array('ExpressCrypto', 'https://expresscrypto.io/signup?referral=31487', false),
				3 => array('Coinbase', 'https://www.coinbase.com/join/negrea_d', false)
			);
	...
	function get_notification($notify, $value, $time, $read = 1, $type = 'none') {
		...
		switch($notify) {
			...
			case 2:
				...
				break;
			case 3:
				$message = '<tr><td class="bg-primary notify-icon"><i class="fa fa-trophy fa-2x fa-fw notify-icon-fa"></i></td><td>Congratulations, you have won <strong>'.$value.' Coins</strong> from <i>Referrals Contest</i>!<br /><span class="small text-muted">'.$time.($read == 0 ? ' <span class="badge badge-info">New</span>' : '').'</span></td></tr>';

				break;
			case 4:
				$message = '<tr><td class="bg-primary notify-icon"><i class="fa fa-users fa-2x fa-fw notify-icon-fa"></i></td><td>Congratulations, you have won <strong>'.$value.' Coins</strong> from <i>Offerwalls Contest</i>!<br /><span class="small text-muted">'.$time.($read == 0 ? ' <span class="badge badge-info">New</span>' : '').'</span></td></tr>';

				break;
			case 5:
				$message = '<tr><td class="bg-warning notify-icon"><i class="fa fa-dollar-sign fa-2x fa-fw notify-icon-fa"></i></td><td>You successfully deposited <strong>$'.$value.'</strong> into <i>Purchase Balance</i>. Thank you!<br /><span class="small text-muted">'.$time.($read == 0 ? ' <span class="badge badge-info">New</span>' : '').'</span></td></tr>';

				break;
			case 6:
				$message = '<tr><td class="bg-primary notify-icon"><i class="fa fa-users fa-2x fa-fw notify-icon-fa"></i></td><td>Congratulations, you have won <strong>'.$value.' Coins</strong> from <i>Shortlinks Contest</i>!<br /><span class="small text-muted">'.$time.($read == 0 ? ' <span class="badge badge-info">New</span>' : '').'</span></td></tr>';

				break;
			case 7:
				$message = '<tr><td class="bg-success notify-icon"><i class="fa fa-dollar-sign fa-2x fa-fw notify-icon-fa"></i></td><td>Your payment request <strong>#'.$value.'</strong> was completed. Funds are now available into your wallet!<br /><span class="small text-muted">'.$time.($read == 0 ? ' <span class="badge badge-info">New</span>' : '').'</span></td></tr>';

				break;
			case 8:
				$message = '<tr><td class="bg-warning notify-icon"><i class="fa fa-star fa-2x fa-fw notify-icon-fa"></i></td><td><b>Congratulations!</b> You won <strong>'.number_format($value, 2).' Coins</strong> from our lottery!<br /><span class="small text-muted">'.$time.($read == 0 ? ' <span class="badge badge-info">New</span>' : '').'</span></td></tr>';
	...
	function ref_commission($user, $referral, $commission, $date = 0) {
		...
		if(!empty($user) && !empty($referral) && !empty($commission)){
			$db->Query("UPDATE `users` SET `tokens`=`tokens`+'".$commission."' WHERE `id`='".$user."'");
	...
	function GenAutoFaucetToken()
	{
		$_SESSION['auto_faucet_token'] = GenToken();
		return $_SESSION['auto_faucet_token'];
	}
