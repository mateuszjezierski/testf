	...
	if($is_valid && !empty($request_uuid) && $status == 'eligible' && (!isset($_REQUEST["debug"]) || $_REQUEST["debug"] == false))
	{
		if($db->QueryGetNumRows("SELECT * FROM `completed_offers` WHERE `survey_id`='".$tx_id."' AND `method`='pollfish' LIMIT 1") == 0)
		{
			...
			if(!empty($user['id'])) {
				...
				$db->Query("INSERT INTO `completed_offers` (`user_id`,`survey_id`,`user_country`,`user_ip`,`revenue`,`reward`,`method`,`timestamp`) VALUES ('".$user['id']."','".$tx_id."','".$user['code']."','".$user['log_ip']."','".$payout."','".$reward_value."','pollfish','".time()."')");
			}
		}
	}
