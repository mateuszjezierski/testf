<?php
	...
	// Security Check
	$secret = $db->QueryFetchArray("SELECT config_value FROM `offerwall_config` WHERE `config_name`='cryptowall_secret'");
    ...
    $fiatPayout = isset($_REQUEST['fiat_payout']) ? $db->EscapeString($_REQUEST['fiat_payout']) : null;
	...
	if(!empty($userId) && $db->QueryGetNumRows("SELECT * FROM `completed_offers` WHERE `survey_id`='".$survey."' LIMIT 1") == 0)
	{
		...
		if(!empty($user['id'])) {
			...
			$db->Query("INSERT INTO `completed_offers` (`user_id`,`survey_id`,`user_country`,`user_ip`,`revenue`,`reward`,`method`,`timestamp`) VALUES ('".$user['id']."','".$survey."','".$country."','".$userIP."','".$fiatPayout."','".$reward."','cryptowall','".time()."')");
		}
	}
