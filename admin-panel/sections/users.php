...
if(isset($_GET['edit']) && $edit['username'] != ""){
	if(isset($_POST['submit'])){
		...
		$tokens = $db->EscapeString($_POST['tokens']);
		...
		if(!isUserID($name)){
			...
			$db->Query("UPDATE `users` SET `username`='".$name."', `email`='".$email."', `country_id`='".$country."', `gender`='".$sex."', `admin`='".$admin."'".$value.", `account_balance`='".$coins."', `tokens`='".$tokens."', `purchase_balance`='".$purchase_balance."', `membership`='".$membership."', `membership_id`='".$membership_id."'".$activate." WHERE `id`='".$id."'");
	...
	$payouts = $db->QueryFetchArray("SELECT SUM(`amount`) AS `cash`, COUNT(`id`) AS `total` FROM `withdrawals` WHERE `user_id`='".$edit['id']."' AND `status`='1'");
			...
			<ul class="info">
				<li>
					<a href="javascript:void(0);">
						<strong><?=number_format(isset($_POST['tokens']) ? $_POST['tokens'] : $edit['tokens'])?></strong>
						<small>Tokens</small>
					</a>
				</li>
				<li>
					<a href="javascript:void(0);">
						<strong><?=number_format(isset($_POST['account_balance']) ? $_POST['account_balance'] : $edit['account_balance'], 2)?> Coins</strong>
						<small>Account Balance</small>
					</a>
				</li>
				<li>
					<a href="javascript:void(0);">
						<strong><?=number_format($payouts['total']).' - $'.number_format($payouts['cash'], 2)?></strong>
				...
				<li>
					<a href="javascript:void(0);">
						<strong><?=number_format($edit['today_revenue'], 2)?> Coins</strong>
						<small>Today Earnings</small>
					</a>
				</li>
				<li>
					<a href="javascript:void(0);">
						<strong><?=number_format($edit['total_revenue'], 2)?> Coins</strong>
		...
		<div class="details grid_4">
			...
			<section>
				<table>
					...
					<tr><th>Revenue:</th><td><i><?=number_format($commissions['commission'], 2)?> Tokens (<?=number_format($commissions['total'])?>)</i></td></tr>
				</table>
			</section>
			<h2>Deposits</h2>
			<section>
				<table>
					...
					?>
					<tr><th>Deposits:</th><td><i><?=number_format($trans['total'])?> deposits</i> - <font style="color:green">$<?=number_format($trans['amount'], 2)?></font></td><td style="width:150px"></td></tr>
				</table>
			</section>
		</div>
		...
		<form action="" method="post" class="box">
			<div class="header">
				...
				<div class="content">
					...
					<div class="row">
						<label><strong>Tokens</strong></label>
						<div><input type="text" name="tokens" value="<?=(isset($_POST['tokens']) ? $_POST['tokens'] : $edit['tokens'])?>" required="required" /></div>
					</div>
					<div class="row">
						<label><strong>Account Balance</strong><small>Coins</small></label>
						<div><input type="text" name="coins" value="<?=(isset($_POST['coins']) ? $_POST['coins'] : $edit['account_balance'])?>" required="required" /></div>
					</div>
					<div class="row">
						<label><strong>Purchase Balance</strong><small>USD</small></label>
			...
			<table class="styled" style="text-align:center">
				<thead>
					<tr>
						...
						<th>Email</th>
						<th>Country</th>
						<th><a href="index.php?x=users&sort=<?=($sorting == 'desc' ? 'asc' : 'desc')?>&oby=4">Tokens <img src="img/elements/table/sorting<?=($sorting == 'asc' && $order_by == 4 ? '-asc' : ($sorting == 'desc' && $order_by == 4 ? '-desc' : ''))?>.png" border="0" /></a></th>
				...
				<tbody>
				<?php
					$users = $db->QueryFetchArrayAll("SELECT id, username, email, account_balance, tokens, membership, country_id, admin, total_claims, disabled FROM users".$db_value." ORDER BY ".$order_value." LIMIT ".$start.",".$limit."");
				...
				?>	
					<tr>
						...
						<td><?=number_format($user['tokens'])?> Tokens</td>
						<td><?=number_format($user['account_balance'], 2)?> Coins</td>
