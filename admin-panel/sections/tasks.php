<?php
	...
	if(isset($_GET['edit'])){
		...
		if(!empty($job['uid'])){
			...
			elseif($job['type'] == 2)
			{
				$db->Query("UPDATE `users` SET `tokens`=`tokens`+'".$job['reward']."' WHERE `id`='".$job['uid']."'");
			}
			else
			{
		...
		<form action="" method="post" class="box">
			...
			<div class="content">
				...
				<div class="row">
					...
					<div><select name="type"><option value="0">Coins</option><option value="1"<?=(!isset($_POST['type']) && $edit['type'] == 1 ? ' selected' : (isset($_POST['type']) && $_POST['type'] == 1 ? ' selected' : ''))?>>Membership Days</option><option value="2"<?=(!isset($_POST['type']) && $edit['type'] == 2 ? ' selected' : (isset($_POST['type']) && $_POST['type'] == 2 ? ' selected' : ''))?>>Faucet Tokens</option></select></div>
				</div>
				<div class="row">
	...
	<div class="grid_12">
		<form action="" method="post" class="box">
			...
			<div class="content">
				...
				<div class="row">
					<label><strong>Reward Type</strong></label>
					<div><select name="type"><option value="0">Coins</option><option value="1">Membership Days</option><option value="2">Faucet Tokens</option></select></div>
				</div>
				<div class="row">
					<label><strong>Reward Membership</strong><small>Only for "Membership Days" reward</small></label>
...
<section id="content" class="container_12 clearfix">
	<h1 class="grid_12">Completed Tasks</h1>
	<div class="grid_12">
		<div class="box">
			<form method="POST">
			<table class="styled">
				...
				<tbody>
					...
					<tr>
						...
						<td><a href="index.php?x=tasks&edit=<?=$job['job_id']?>"><?=$job['job_id']?></a></td>
						<td><?=$job['requirement']?></td>
						<td><?=($job['type'] == 1 ? 'Membership<br />'.number_format($job['reward'], 0).' days' : ($job['type'] == 2 ? number_format($job['reward']).' Faucet Tokens' : $job['reward'].' Coins'))?></td>
						...
						<td class="center">
							<?php if($job['status'] == 0) { ?>
								<a href="index.php?x=tasks&pending&approve=<?=$job['id']?>" class="button small grey tooltip" data-gravity=s title="Approve"><i class="icon-ok"></i></a>
								<a href="index.php?x=tasks&pending&reject=<?=$job['id']?>" class="button small grey tooltip" data-gravity=s title="Reject"><i class="icon-remove"></i></a>
							<?php 
								} else {
									echo 'N/A';
								}
			...
			<table class="styled">
				...
				<tbody>
					...
					<tr>
						<td><?=$job['id']?></td>
						<td><?=$job['title']?></td>
						<td><?=($job['type'] == 1 ? number_format($job['reward'], 0).' Membership Days' : ($job['type'] == 2 ? number_format($job['reward']).' Faucet Tokens' : $job['reward'].' Coins'))?></td>
						<td><?=date('d M Y - H:i', $job['time'])?></td>
						<td class="center">
							<a href="index.php?x=tasks&edit=<?=$job['id']?>" class="button small grey tooltip" data-gravity=s title="Edit"><i class="icon-pencil"></i></a>
							<a href="index.php?x=tasks&del=<?=$job['id']?>" class="button small grey tooltip" data-gravity=s title="Remove"><i class="icon-remove"></i></a>
						</td>
					</tr>
					<?php }?>
				</tbody>
