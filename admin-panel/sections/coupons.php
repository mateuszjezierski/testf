	...
	<div class="grid_12">
		<form action="" method="post" class="box">
			...
			<div class="content">
				...
				<div class="row">
					<label><strong>Coupon Type</strong><small>Chose between Coins or USD</small></label>
					<div><select name="type"><option value="0">Account Balance (Coins)</option><option value="1"<?=(!isset($_POST['type']) && $edit['type'] == 1 ? ' selected' : (isset($_POST['type']) && $_POST['type'] == 1 ? ' selected' : ''))?>>Purchase Balance (USD)</option><option value="2"<?=(!isset($_POST['type']) && $edit['type'] == 2 ? ' selected' : (isset($_POST['type']) && $_POST['type'] == 2 ? ' selected' : ''))?>>Faucet Tokens</option></select></div>
				</div>
				<div class="row">
					<label><strong>Coupon Value</strong><small>Coins or USD</small></label>
			...
			<table class="styled">
				...
				<tbody>
					...
					<tr>
						<td><?=$coupon['id']?></td>
						<td><?=$coupon['code']?></td>
						<td><?=($coupon['type'] == 1 ? '$'.$coupon['value'] : ($coupon['type'] == 2 ? number_format($coupon['value'], 0).' Tokens' : $coupon['value'].' Coins'))?></td>
	...
	<form action="" method="post" class="box">
		...
		<div class="content">
			...
			<div class="row">
				...
				<div><select name="type"><option value="0">Account Balance (Coins)</option><option value="1">Purchase Balance (USD)</option><option value="2">Faucet Tokens</option></select></div>
			</div>
			<div class="row">
				<label><strong>Coupon Value</strong><small>Coins or USD</small></label>
				<div><input type="text" name="value" placeholder="0.00" required="required" /></div>
			</div>
			<div class="row">
				<label><strong>Requirements</strong><small>Claims required to use this coupon</small></label>
