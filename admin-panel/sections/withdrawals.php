...
if(isset($_GET['pay']) && is_numeric($_GET['pay'])){
	...
	$req = $db->QueryFetchArray("SELECT * FROM `withdrawals` WHERE `id`='".$pid."'");
	if(!empty($req['id'])){
		if($req['status'] == 0){
			if($req['method'] == 1) {
				$faucetpay = new FaucetPay($config['fp_api_key'], $req['coin']);
				$fp_result = $faucetpay->send($req['payment_info'], ($req['crypto']*100000000));

				if($fp_result['success'] == true)
				{
					$db->Query("UPDATE `withdrawals` SET `status`='1', `payout_id`='".$fp_result['payout_id']."' WHERE `id`='".$req['id']."'");
					$msg = '<div class="alert success">'.$req['crypto'].' '.strtoupper($req['coin']).' was transfered and request was marked as paid!</a></div>';
			...
			elseif($req['method'] == 3)
			{
				...
				$cb_result = $coinbase->sendPayment($req['payment_info'], $req['crypto'], $req['coin'], 'Withdrawal from '.$config['site_name']);

				if(isset($cb_result['data']['id']) && !empty($cb_result['data']['id']))
				{
					$db->Query("UPDATE `withdrawals` SET `status`='1', `payout_id`='".$cb_result['data']['id']."' WHERE `id`='".$req['id']."'");
					$msg = '<div class="alert success">'.$req['crypto'].' '.strtoupper($req['coin']).' was transfered and request was marked as paid!</a></div>';
				...
				{
					$msg = '<div class="alert error">System wasn\'t able to send requested amount, check your Coinbase account!</div>';
				}
			}
		}
	}else{
	...
	<div class="grid_6">
		<div class="box">
            ...
            <div class="content">
				...
				<p><strong>Coins Amount:</strong> <font color="blue"><b><?=number_format($info['bits'], 2)?></b></font></p>
				<p><strong>Withdraw Method:</strong> <?=paymentMethod($info['method'])?></p>
				<p><strong>Withdraw Address:</strong> <?=$info['payment_info']?></p>
				<p><strong>Coins Value:</strong> <font color="green"><b>$<?=number_format($info['amount'], 2)?> (<?=(number_format($info['crypto'], 8).' '.strtoupper($info['coin']))?>)</b></font></p>
				<hr>
				<p align="center">
...
<section id="content" class="container_12 clearfix ui-sortable"><?=$msg?>
	...
	<div class="grid_12">
		<div class="box">
			<table class="styled" style="text-align:center">
				...
				<?php
					...
					$total_pages = $db->QueryGetNumRows("SELECT a.id FROM withdrawals a".$query);
					...
					$reqs = $db->QueryFetchArrayAll("SELECT a.*, b.username, b.membership_id, b.last_activity, b.disabled, c.membership AS mem_name FROM withdrawals a LEFT JOIN users b ON b.id = a.user_id LEFT JOIN memberships c ON c.id = b.membership_id".$query." ORDER BY a.time DESC LIMIT ".$start.",".$limit."");
				...
				?>	
					<tr>
						...
						<td><?=number_format($req['bits'], 2)?> Coins</td>
						<td><b>$<?=number_format($req['amount'], 2)?> - <?=(number_format($req['crypto'], 8).' '.strtoupper($req['coin']))?></b></td>
						...
						<td class="center">
						<?php if($req['status'] == 0) { ?>
							<a href="index.php?x=withdrawals<?=$pageURL?>&p=<?=$page?>&pay=<?=$req['id']?>" onclick="return confirm('You sure you want to proccess this request?');" class="button small blue tooltip" title="Send Payment"><i class="icon-ok-sign"></i></a>
							<a href="index.php?x=withdrawals<?=$pageURL?>&p=<?=$page?>&refund=<?=$req['id']?>" onclick="return confirm('You sure you want to refund this request?');" class="button small grey tooltip" title="Refund"><i class="icon-refresh"></i></a>
							<a href="index.php?x=withdrawals<?=$pageURL?>&p=<?=$page?>&mark=<?=$req['id']?>" onclick="return confirm('You sure you want to mark this request as Paid? Payment must be sent manually.');" class="button small grey tooltip" title="Mark As Complete"><i class="icon-ok"></i></a>
							<a href="index.php?x=withdrawals<?=$pageURL?>&p=<?=$page?>&reject=<?=$req['id']?>" class="button small red tooltip" title="Reject"><i class="icon-remove"></i></a>
						<?php 
							} else { 
								echo 'N/A';
							}
		...
		<div class="alert information">Make sure you have enough funds into FaucetPay or CoinPayments accounts. You won't be able to proccess pending requests if you don't have enough funds!</div>
	</div>
</section>
<?php } ?>