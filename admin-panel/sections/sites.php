<?php
	...
	if(isset($_GET['edit']))
	{
		...
		if(isset($_POST['submit']))
		{
			...
			$db->Query("UPDATE `ptc_websites` SET `user_id`='".$user_id."', `website`='".$url."', `title`='".$title."', `description`='".$description."', `total_visits`='".$clicks."', `daily_limit`='".$limit."', `redirect`='".$redirect."', `status`='".$status."', `ptc_pack`='".$ptc_pack."' WHERE `id`='".$edit['id']."'");
	...
	elseif(isset($_GET['add']))
	{
		if(isset($_POST['submit']))
		{
			...
			$db->Query("INSERT INTO `ptc_websites` (`user_id`,`website`,`title`,`description`,`total_visits`,`daily_limit`,`ptc_pack`,`redirect`,`status`,`added_time`)VALUE('".$data['id']."', '".$url."', '".$title."', '".$description."', '".$clicks."', '".$limit."', '".$ptc_pack."', '".$redirect."', '1','".time()."')");
		...
		<div class="grid_12">
		  <form action="" method="post" class="box">
			...
			<div class="content">
				...
				<div class="row">
					<label><strong>Website Description</strong></label>
					<div><textarea name="description" required><?=(isset($_POST['description']) ? $db->EscapeString($_POST['description']) : $edit['description'])?></textarea></div>
				</div>
				<div class="row">
					...
					<div>
						<select name="ptc_pack">
							<?php
								...
								{
									echo '<option value="'.$pack['id'].'"'.($edit['ptc_pack'] == $pack['id'] ? ' selected' : '').'>'.$pack['time'].' seconds - $'.$pack['price'].'</option>';
								}
							?>
						</select>
					</div>
		...
		<div class="grid_12">
		  <form action="" method="post" class="box">
			...
			<div class="content">
				...
				<div class="row">
					<label><strong>Website Description</strong></label>
					<div><textarea name="description" placeholder="Add your website description" required><?=(isset($_POST['description']) ? $db->EscapeString($_POST['description']) : '')?></textarea></div>
				</div>
				<div class="row">
					<label><strong>Total Visits</strong></label>
					...
					<div>
						<select name="ptc_pack">
							<?php
								...
								foreach($ptcPacks as $pack)
								{
									echo '<option value="'.$pack['id'].'">'.$pack['time'].' seconds - $'.$pack['price'].'</option>';
								}
							?>
						</select>
					</div>
			...
			<form action="" method="post" class="box">
				<div class="header">
					...
					<div class="content">
						...
						<div class="row">
							<label><strong>Price per Visit</strong><small>USD</small></label>
							<div><input type="text" name="price" value="<?=(isset($_GET['pack_edit']) && !empty($edit['price']) ? $edit['price'] : '')?>" placeholder="0.0001" required="required" /></div>
						</div>
						<div class="row">
							<label><strong>Reward</strong><small>Coins</small></label>
				...
				<table class="styled" style="text-align: center">
					...
					?>	
						<tr>
							...
							<td>$<?=(number_format($website['price'], 4).' per visits')?></td>
