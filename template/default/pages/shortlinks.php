		...
		<div class="my-3 ml-2 p-3 bg-white rounded box-shadow box-style">
		  <div id="grey-box">
			...
			<div class="content">
				...
				<span id="status"><div class="alert alert-info text-center"><?php echo lang_rep($lang['l_457'], array('-SUM-' => $data['shortlinks_contest'])); ?></div></span>
						  ...
						  <?php
							...
							else
							{
								...
								foreach($shortlinks as $shortlink) {
									...
									if($proxy === false)
									{
										echo '<tr><td class="align-middle">'.$shortlink['name'].'</td><td class="align-middle text-center"><b class="badge badge-dark">'.$shortlink['reward'].' '.$lang['l_530'].'</b></td><td class="align-middle text-center"><b class="badge badge-dark">'.$remain.'/'.$shortlink['daily_limit'].'</b></td><td class="text-right">'.($remain > 0 ? '<button onclick="goShortlink(\''.$shortlink['id'].'\');" class="btn btn-success btn-sm"  type="submit">'.$lang['l_224'].' <i class="fa fa-external-link fa-fw"></i></button>' : '<span id="short_'.$shortlink['id'].'" class="badge badge-light p-2" data-seconds-left="'.$remTime.'"></span><div class="badge badge-light p-2" id="staticTime_'.$shortlink['id'].'">'.gmdate('H:i:s', $remTime).'</div>').'</td></tr>';
									}
								}
							}
						  ?>
