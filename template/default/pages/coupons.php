<?php
	...
	if(isset($_POST['submit']))
	{
		...
		else
		{
			if($ext['type'] == 1)
			{
				$db->Query("UPDATE `users` SET `purchase_balance`=`purchase_balance`+'".$ext['value']."' WHERE `id`='".$data['id']."'");
			}
			elseif($ext['type'] == 2)
			{
				$db->Query("UPDATE `users` SET `tokens`=`tokens`+'".$ext['value']."' WHERE `id`='".$data['id']."'");
			}
			else
			{
			...
			$errMessage = '<div class="alert alert-success" role="alert"><i class="fa fa-check"></i> '.lang_rep($lang['l_431'], array('-NUM-' => ($ext['type'] == 1 ? '$'.$ext['value'] : ($ext['type'] == 2 ? $ext['value'].' '.$lang['l_530'] : $ext['value'].' '.$lang['l_337'])))).'</div>';
		}
	}
?> 
					...
					<table class="table table-striped table-sm table-responsive-sm text-center">
						...
						?>	
							<tr>
								...
								<td><?=($coupon['type'] == 1 ? '$'.$coupon['value'] : ($coupon['type'] == 2 ? $coupon['value'].' '.$lang['l_530'] : $coupon['value'].' '.$lang['l_337']))?></td>
