<?php
	...
	if (isset($_POST['buy_ticket'])) {
		...
		$lottery_price = ($config['lottery_price']*$tickets);
		$fiat_value = number_format(($lottery_price*$config['bits_rate']), 4);

		if ($data['account_balance'] < $lottery_price && $source == 0) 
		{
			$errMessage = '<div class="alert alert-danger" role="alert">'.$lang['l_164'].'</div>';
		}
		elseif ($data['purchase_balance'] < $fiat_value && $source == 1) 
		{
			$errMessage = '<div class="alert alert-danger" role="alert">'.$lang['l_164'].'</div>';
		}
		else 
		{
			...
			if($source == 1)
			{
				$db->Query("UPDATE `users` SET `purchase_balance`=`purchase_balance`-'".$fiat_value."' WHERE `id`='".$data['id']."'");
				$db->Query("INSERT INTO `user_transactions` (`user_id`,`type`,`value`,`price`,`date`) VALUES('".$data['id']."', '4', '".$tickets."', '".$fiat_value."', '".time()."') ");
				$data['purchase_balance'] = ($data['purchase_balance']-$fiat_value);
			}
			else
			{
		...
		<div class="my-3 p-3 bg-white rounded box-shadow box-style">
		  <div id="grey-box">
			...
			<div class="content">
				...
				<div class="box">
					<small><?php echo lang_rep(($config['lottery_duration'] == 0 ? $lang['l_273'] : $lang['l_449']), array('-PRICE-' => $config['lottery_price'], '-SAT-' => number_format($config['lottery_price']*$config['bits_rate'], 4))); ?></small>
				</div>
