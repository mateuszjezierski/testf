...
?> 
								...
								if(!empty($url) && !empty($pack) && $_FILES['cons_image']['name']){
									...
									if($pack['price'] > $data['purchase_balance']) {
										...
										if(!$copied){
											...
											$expiration = ($pack['value']*86400)+time();
											...
											$db->Query("INSERT INTO `user_transactions` (`user_id`,`type`,`value`,`price`,`date`) VALUES('".$data['id']."', '3', '".$pack['value']."', '".$pack['price']."', '".time()."') ");
						...
						<form method="post" enctype="multipart/form-data">
						  ...
						  <div class="form-row">
							<div class="form-group col-md-6">
							  ...
							  <select id="position" name="position" onchange="getOptions()" class="custom-select">
							...
							<div class="form-group col-md-6">
							  ...
							  <select id="size" name="size" onchange="getOptions()" class="custom-select">
								...
								<option value="1">728x90 px</option>
							  </select>
							</div>
						  </div>
						  <div class="form-row">
							<div class="form-group col-md-6">
							  <label for="pack">Advertise by</label>
							  <select id="pack" name="pack" onchange="getOptions()" class="custom-select">
							...
							<div class="form-group col-md-6">
							  ...
							  <select id="bPacks" name="pack" class="custom-select">
								<?php
									$packs = $db->QueryFetchArrayAll("SELECT * FROM `ad_packs` WHERE `type`='0' AND `pack`='0' ORDER BY `price` ASC");
									foreach($packs as $pack){
										echo '<option value="'.$pack['id'].'"'.(isset($_POST['pack']) && $_POST['pack'] == $pack['id'] ? ' selected' : '').'>'.$pack['value'].' '.$lang['l_234'].' - $'.$pack['price'].'</option>';
									}
								?>
							  </select>
							</div>
					...
					<div class="content">
						<table class="table table-striped table-sm table-responsive-sm">
							<thead class="thead-dark text-center">
								<tr><th><?=$lang['l_187']?></th><th><?=$lang['l_188']?></th><th><?=$lang['l_189']?></th><th><?=$lang['l_196']?></th><th>Position</th><th></th></tr>
							</thead>
							<tbody class="table-primary text-dark text-center">
								<?php
									$banners = $db->QueryFetchArrayAll("SELECT a.*, b.price, b.value, b.pack, b.position FROM banners a LEFT JOIN ad_packs b ON b.id = a.ad_pack WHERE a.user = '".$data['id']."' AND a.status > '0'");
									...
									foreach($banners as $banner){
										$status = ($banner['pack'] == '0' && $banner['expiration'] > time() || $banner['pack'] == '1' && $banner['value'] > $banner['clicks'] || $banner['pack'] == '2' && $banner['value'] > $banner['views'] ? '<i class="fa fa-play text-success" title="'.$lang['l_68'].'"></i>' : ($banner['status'] == 2 ? '<i class="fa fa-times text-danger" title="'.$lang['l_201'].'"></i>' : '<i class="fa fa-stop text-danger" title="'.$lang['l_69'].'"></i>'));
								?>
									<tr><td><a href="<?=$banner['site_url']?>" title="<?=$banner['site_url']?>" class="img-fluid" target="_blank"><img style="max-width: 225px" src="<?=$config['secure_url'].$banner['banner_url']?>" width="280" border="0" /></a></td><td><?=number_format($banner['views'])?></td><td><?=number_format($banner['clicks'])?></td><td align="center"><?=($banner['type'] == 1 ? '728x90' : '468x60')?></td><td><?=($banner['position'] == 1 ? 'Header' : 'Footer')?></td><td><?=$status?></td></tr>
								<?php } ?>				
							</tbody>
						</table>
					</div>
	...
	<script>
		function getOptions(){
			...
			var c=$("#pack").val();
			var d=$("#position").val();
			$.get('system/ajax.php?a=bannerPacks&type='+b+'&pack='+c+'&position='+d,function(a){
				$('#bPacks').html(a)
			})
		} 
	</script>