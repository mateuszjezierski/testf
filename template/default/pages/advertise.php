<?php
	...
	if(isset($_POST['submit'])){
		...
		elseif(strlen($title) < 4 || strlen($title) > 60)
		{
			$errMessage = '<div class="alert alert-danger" role="alert">'.$lang['l_522'].'</div>';
		}
		elseif(strlen($description) < 20 || strlen($description) > 120)
		{
			$errMessage = '<div class="alert alert-danger" role="alert">'.$lang['l_523'].'</div>';
		}
		...
		else
		{
			...
			{
				$price = $ptc_pack['price'] * $ad_visits;
				if($redirect)
				{
				...
				else
				{
					...
					$db->Query("INSERT INTO `ptc_websites` (`user_id`,`website`,`title`,`description`,`daily_limit`,`total_visits`,`ptc_pack`,`redirect`,`status`,`added_time`) VALUES('".$data['id']."', '".$url."', '".$title."', '".$description."', '".$daily_clicks."', '".$ad_visits."', '".$ptc_pack['id']."', '".$redirect."', '1', '".time()."')");
					...
					<form method="post">
						<div class="form-row">
							<div class="form-group col-md-6">
							  ...
							  <input type="text" class="form-control" name="url" id="url" value="<?php echo (isset($_POST['url']) ? $db->EscapeString($_POST['url']) : ''); ?>" placeholder="https://" required />
							</div>
							<div class="form-group col-md-6">
							  <label for="title"><?=$lang['l_91']?></label>
							  <input type="text" class="form-control" name="title" id="title" value="<?php echo (isset($_POST['title']) ? $db->EscapeString($_POST['title']) : ''); ?>" maxlength="60" placeholder="<?php echo $lang['l_526']; ?>" required />
							</div>
						</div>
                        <div class="form-row">
							<div class="form-group col-md-12">
							  <label for="description"><?=$lang['l_524']?></label>
							  <textarea class="form-control" name="description" id="description" maxlength="120" placeholder="<?=$lang['l_525']?>" required><?php echo (isset($_POST['description']) ? $db->EscapeString($_POST['description']) : ''); ?></textarea>
							</div>
						</div>
						<div class="form-row">
							<div class="form-group col-md-6">
							  ...
							  <select name="ad_pack" id="ad_pack" class="custom-select" onchange="get_price()" >
								<?php
									...
									foreach($daily_packs as $pack) {
										echo '<option value="'.$pack['id'].'">'.$pack['time'].' '.$lang['l_182'].' - $'.$pack['price'].'</option>';
									}
								?>
							  </select>
							</div>
						...
						<div class="price_block">
							<span class="text" id="priceBlock"><?php echo lang_rep($lang['l_97'], array('-PRICE-' => '$0.00 ')); ?></span>
						...
						<tbody class="table-primary text-dark">
							<?php
								...
								else
								{
									...
									{
										echo '<tr><td>'.$website['id'].'</td><td><a href="'.$website['website'].'" title="'.$website['description'].'" target="_blank">'.$website['title'].'</a></td><td>'.number_format($website['total_visits']).' '.$lang['l_295'].' x '.$website['time'].' '.$lang['l_182'].'</td><td>'.number_format($website['received_today']).' / '.($website['daily_limit'] == 0 ? '&#8734;' : number_format($website['daily_limit'])).'</td><td>'.number_format($website['total_visits'] - $website['received']).' '.$lang['l_295'].'</td></tr>';
									}
								}
							?>
						</tbody>
