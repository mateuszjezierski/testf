					...
					?>
						<div class="card mb-2">
						  <div class="card-header">
							...
							<p class="text-danger text-center mb-0"><?=$lang['l_02']?>: <?=$job['id']?> - <?=$lang['l_03']?>: <?=($job['type'] == 1 ? number_format($job['reward'], 0).' '.($data['membership_id'] == $job['membership'] ? $job['mem_name'] : ($data['membership_id'] > 1 ? $data['mem_name'] : $job['mem_name'])).' '.$lang['l_234'] : ($job['type'] == 2 ? number_format($job['reward']).' '.$lang['l_531'] : $job['reward'].' '.$lang['l_337']))?></p>
						  </div>
						  <div class="card-body text-dark">
							...
							<?php
									...
									echo '<div class="alert alert-success text-center mb-0" role="alert">'.lang_rep($lang['l_06'], array('-REWARD-' => ($job_status['type'] == 1 ? number_format($job_status['reward'], 0).' '.$lang['l_01'] : ($job_status['type'] == 2 ? $job_status['reward'].' '.$lang['l_531'] : $job_status['reward'].' '.$lang['l_337'])))).'</div>';
								}
							?>
						  </div>
						</div>
