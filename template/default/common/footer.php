<?php
	...
	if($internal_banner)
	{
		$ad_banner = $db->QueryFetchArray("SELECT a.id, a.banner_url FROM banners a LEFT JOIN ad_packs b ON b.id = a.ad_pack WHERE b.position = '0' AND a.status = '1' AND (a.type = '0' OR a.type = '1') AND (b.pack = '0' AND a.expiration > '".time()."' OR b.pack = '1' AND b.value > a.clicks OR b.pack = '2' AND b.value > a.views) ORDER BY rand() LIMIT 1");
		if(!empty($ad_banner['id']))
		{
			$db->Query("UPDATE `banners` SET `views`=`views`+'1' WHERE `id`='".$ad_banner['id']."'");
			
			$banner_code = '<div class="container"><div class="mx-auto"><div class="my-3 p-3 bg-white rounded box-shadow box-style text-center d-flex justify-content-center"><a href="'.$config['secure_url'].'/?go_banner='.$ad_banner['id'].'&hash='.$token.'" target="_blank"><img src="'.$config['secure_url'].$ad_banner['banner_url'].'" class="img-fluid" alt="Banner #'.$ad_banner['id'].'" border="0" /></a></div></div></div>';
		}
	}
...
?>
	...
	<footer class="container my-3">
		<nav class="navbar navbar-expand-sm navbar-dark rounded">
		 <div class="container">
		  <span class="navbar-brand copyright"><?=eval(base64_decode(''))?></span>
			...
			<ul class="navbar-nav ml-auto">
			  <li class="nav-item dropup">
				...
				<div class="dropdown-menu" aria-labelledby="footer_menu">
					...
					<a class="dropdown-item" href="<?=GenerateURL('payments')?>"><i class="fas fa-money-bill fa-fw"></i> <?php echo $lang['l_439']; ?></a>
...
<?php } ?>
	...
	<!-- Ultimate Auto Faucet v<?php echo $config['version']; ?> - Developed by www.ScriptStore.xyz -->
  </body>
</html>